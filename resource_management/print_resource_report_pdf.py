...
from resource_management.models import ResourceBooking
...
class MyPrintResourceReport:
    ...
    def print_report(self,filterstr):
        ...
        buffer = self.buffer
        doc = SimpleDocTemplate(f'media/reports/resources_{facility_uuid}.pdf',
                                rightMargin=40,
                                leftMargin=40,
                                topMargin=100,
                                bottomMargin=72,
        ...
        table_contents = [
                                [
                                    ...
                                    Paragraph('<b>Resource</b>',styles['Normal']),
        ...
        for booking in booking_list:
            ...
            table_contents.append(
                    [
                        ...
                        Paragraph(f'{ booking.resource.resource }',styles['Normal']),
                        Paragraph(f'{ booking.number_of_resources }',styles['Normal']),
        ...
        modules_table = Table(
            table_contents,
            colWidths=[
                4.0 * cm, 3.0 * cm, 4.0 * cm, 3.0 * cm,
                2.5 * cm, 2.5 * cm, 2.5 * cm, 4.0 * cm,
            ]
        )
        ...
        pdf_paths = [f'media/reports/resources_{facility_uuid}.pdf']
        ...
        writer.write(f'media/reports/resources_{facility_uuid}.pdf')
      
        pdf_file = f'media/reports/resources_{facility_uuid}.pdf'
        watermark = "media/letter_head_landscape.pdf"
        watermark_other_pages = "media/letter_head_landscape.pdf"
        merged_file = f"media/reports/resources_{facility_uuid}_merged.pdf"
        ...
        del_letter_tpl = f'reports/resources_{facility_uuid}.pdf'
        file_path = os.path.join(settings.MEDIA_ROOT, del_letter_tpl)

        if os.path.isfile(file_path):
            os.remove(file_path)

        return f"reports/resources_{facility_uuid}_merged.pdf"
