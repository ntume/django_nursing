...
class MyPrintReg:
    ...
    def print_registrations(self,pk):
        rsvps = EventCompanyRSVP.objects.filter(event_id = pk)
        ...
        table_data = [
                      [Paragraph('Event',styles['Heading3']),Paragraph(' {}'.format(event.title),styles['Normal'])],
                      [Paragraph('Date',styles['Heading3']),Paragraph(' {}'.format(event.event_date),styles['Normal'])],
                      ]
        ...
        table_data = [['Company','Approved','Paid','Registration Date']]

        for r in rsvps:
            table_data.append(['{}'.format(r.company.company_name),'{}'.format(r.approved),'{}'.format(r.paid),'{}'.format(r.created_at.strftime("%Y-%m-%d"))])

        print(table_data)
        marks_table = Table(table_data,colWidths=[5 * cm, 4.5 * cm])
