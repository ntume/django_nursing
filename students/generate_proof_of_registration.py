...
class MyPrintProofOfRegistration:
    ...
    def print_pop(self,student_registration):
        
        global qrcode_image_file  
        
        buffer = self.buffer
        doc = SimpleDocTemplate(f'media/college/pops/{student_registration.id}.pdf',
        rightMargin=60,
        leftMargin=60,
        topMargin=100,
        bottomMargin=80,
        ...
        input_data = f"https://sims.ahcnc.co.za/api/student/registration/verification/{ student_registration.slug }/verification"
        ...
        img.save(f'media/students/registration/qr_code/{student_registration.slug}.jpeg')
        ...
        table_data = [[Paragraph(f'<b>Proof of Registration</b><br/><br/>Date of Registration: {student_registration.registration_date}<br/>',smallStyle),]]     
        title_table = Table(table_data,colWidths=[18.5 * cm])
        elements.append(title_table)
        elements.append(Paragraph("<br/>", styles['Heading3']))
        ...
        table_data = [
                      [Paragraph('Student Name/s',smallStyle),Paragraph(f'{student_registration.student_learning_programme.student.first_name}',smallStyle),],
                      [Paragraph('Student Surname',smallStyle),Paragraph(f'{student_registration.student_learning_programme.student.last_name}',smallStyle),],
                      [Paragraph('Student ID Number',smallStyle),Paragraph(f'{student_registration.student_learning_programme.student.id_number}',smallStyle),],
                      ...
                      [Paragraph('Student Postal Address',smallStyle),Paragraph(f'{student_registration.student_learning_programme.student.postal_address_1}, {student_registration.student_learning_programme.student.postal_address_2}, {student_registration.student_learning_programme.student.postal_address_3}',smallStyle),],
                      [Paragraph('Student E-mail Address',smallStyle),Paragraph(f'{student_registration.student_learning_programme.student.email}',smallStyle),],
        ...
        table_data = [[Paragraph(f'<b>This serves as proof of registration for the following purposes: Bursaries, Loans, Medical Aid Schemes and Income Tax.</b>',smallStyle),],
                      [Paragraph(f"<b>It is the student's responsibility to ensure that the registration is correct for the current academic year.</b>",smallStyle),],]     
        title_table = Table(table_data,colWidths=[18.5 * cm])
        ...
        table_data = [[Paragraph(f'<b>For any inquiries, please contact Mx Carla Louw by email at nursingcollege@africahealthcare.co.za</b>',smallStyle),],]     
        title_table = Table(table_data,colWidths=[18.5 * cm])
        title_table.setStyle(TableStyle([
                ('INNERGRID', (0,0), (-1,-1), 0.25, colors.black),
                ('BOX', (0,0), (-1,-1), 0.25, colors.black),
        ...
        table_data = [
                [Paragraph('',pageTextStyleCenter),Paragraph(f'<br/><br/><br/><br/><br/><img src="media/students/registration/qr_code/{student_registration.slug}.jpeg" height = "50" width="50"/><br/>Scan Code to Verify',pageTextStyleCenter),Paragraph('',pageTextStyleCenter),],
            ]

        signature_table = Table(table_data,colWidths=[6 * cm, 6.5 * cm, 6 * cm])
        
        elements.append(signature_table)
        elements.append(Paragraph("<br/><br/><br/>", styles['Heading3']))
        
        doc.build(elements,canvasmaker=NumberedCanvas)

        pdf_paths = [f'media/college/pops/{student_registration.id}.pdf']
        ...
        writer.write(f'media/college/pops/{student_registration.id}.pdf')
      
        pdf_file = f'media/college/pops/{student_registration.id}.pdf'
        watermark = "media/letter_head.pdf"
        watermark_other_pages = "media/letter_head.pdf"
        merged_file = f"media/college/pops/{student_registration.id}_merged.pdf"
        ...
        del_letter_tpl = f'college/pops/{student_registration.id}.pdf'
        file_path = os.path.join(settings.MEDIA_ROOT, del_letter_tpl)

        if os.path.isfile(file_path):
            os.remove(file_path)

        return f"college/pops/{student_registration.id}_merged.pdf"
